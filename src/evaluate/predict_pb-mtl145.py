...
for targ_ct, target_id in enumerate(test_lakes): #for each target lake
	...
	for i in range(145):
		csv.append(",".join(['nhdhr_'+str(target_id), 'nhdhr_'+str(lake_df.iloc[i]['site_id']), str(lake_df.iloc[i]['rmse']), str(lake_df.iloc[i]['rmse_pred'])]+ [str(x) for x in lake_df.iloc[i][feats].values]))
