...
tst_inds = [i+2 for i in tst_inds]
tst_lakes = np.array([lakenames[i] for i in tst_inds])
trn_lakes = np.delete(lakenames,tst_inds)
if not os.path.exists("./ealstm_trn_data3.npy"):
    (trn_data, _) = buildCtlstmLakeData(trn_lakes,\
                                        seq_length, n_features,\
                                        win_shift = win_shift, begin_loss_ind = begin_loss_ind,\
                                        verbose=True) 

    np.save("./ealstm_trn_data3.npy",trn_data)
else:
    trn_data = torch.from_numpy(np.load("./ealstm_trn_data3.npy"))

if not os.path.exists("./ealstm_val_data3.npy"):
...
for epoch in range(n_eps):
    ...
    # 
            ...
            if avg_mse < min_train_rmse:
                ...
                print("model saved")
                save_path = "../../models/3cEALSTM_"+str(n_hidden)+"hid_"+str(num_layers)+"_final"
                saveModel(lstm_net.state_dict(), optimizer.state_dict(), save_path)
                ep_since_min = 0
